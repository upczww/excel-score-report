...
The generator reads only the first worksheet and expects this fixed layout:
...
Question labels come from row 1. Free-response target scores come from row 2, columns S onward. Rows with a blank `姓名` are skipped.
...
If a workbook does not match this layout, ask the user to adapt it or provide the blank template. Do not guess a column mapping when doing so could assign scores to the wrong questions or students.
